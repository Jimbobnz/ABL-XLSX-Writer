...
FUNCTION CRC32 RETURNS INT64 (INPUT mpData AS MEMPTR):
    ...
    in_Size = GET-SIZE(mpData).
 
    DO in_loop = 1 TO in_Size:
        in_temp = BinaryXOR(crc_value, GET-BYTE(mpData, in_loop )).
        crc_value = BinaryXOR( ShiftRight(crc_value, 8), crc32_tab[BinaryAND(in_temp,0x00ff) + 1 ] ).
    END.
...
DEFINE VARIABLE chStringData    AS CHARACTER   NO-UNDO LABEL 'CRC32'.                                
DEFINE VARIABLE inLength        AS INTEGER     NO-UNDO.
 
UPDATE chStringData FORMAT "x(16)".
...
chStringData = CAPS( intToHex(CRC32(INPUT mpData)) ).
set-size(mpData) = 0. /** ALWAYS ZERO YOUR MEMPTRs **/

DISPLAY etime 'milliseconds'.
UPDATE chStringData FORMAT "x(16)" .
