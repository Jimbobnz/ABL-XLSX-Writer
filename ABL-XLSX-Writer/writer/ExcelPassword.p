...
FUNCTION ShiftLeft RETURNS INTEGER(in_Operand_A AS INTEGER,
                                   in_Operand_B AS INTEGER):
  /** Bit Shift Left **/
  RETURN INTEGER( in_Operand_A * EXP(2,in_Operand_B)  ).

END FUNCTION.
 
FUNCTION ShiftRight RETURNS INTEGER(in_Operand_A AS INTEGER,
                                    in_Operand_B AS INTEGER):
  /** Bit Shift Right **/
/*   RETURN INTEGER( TRUNCATE( in_Operand_A / in_EXP[ in_Operand_B + 1], 0 ) ). */

   RETURN INTEGER( TRUNCATE( in_Operand_A / EXP(2,in_Operand_B), 0 ) ).

END FUNCTION.

FUNCTION BinaryAND RETURNS INTEGER (in_Operand_A AS INTEGER,
                                    in_Operand_B AS INTEGER):  
...
FUNCTION BinaryOR RETURNS INTEGER(in_Operand_A  AS INTEGER,
                                  in_Operand_B  AS INTEGER):
...
FUNCTION BinaryXOR RETURNS INTEGER(INPUT intOperand1 AS INTEGER,
    ...
    DO iByteLoop = 1 TO 16: /* as processing a double byte character */
...
FUNCTION dec2hex RETURNS character ( INPUT iNumber AS INTEGER ) :
...
FUNCTION _encode_password RETURNS CHARACTER(INPUT ch_Password AS CHARACTER):
    ...
    DEFINE VARIABLE in_char         AS INTEGER     NO-UNDO.
    DEFINE VARIABLE in_password     AS INTEGER     NO-UNDO.
    DEFINE VARIABLE in_low_15       AS INTEGER     NO-UNDO.
    DEFINE VARIABLE in_high_15      AS INTEGER     NO-UNDO.
